...
// استبدلنا axios العادي بملف الـ API بتاعنا
...
const Login = () => {
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    try {
      // هنا استخدمنا الـ API الموحد وشلنا الرابط الطويل (localhost)
  ...
  return (
    <div className={`relative min-h-screen w-full flex items-center justify-center transition-colors duration-500 font-sans p-4 overflow-hidden 
      ...
      {isDarkMode && (
        <>
          <div className="absolute top-[-10%] left-[-10%] w-125 h-125 bg-blue-600/20 rounded-full blur-[120px] animate-pulse" />
          <div className="absolute bottom-[-10%] right-[-10%] w-150 h-150 bg-purple-600/10 rounded-full blur-[120px] animate-bounce" style={{ animationDuration: '8s' }} />
        </>
      )}

      <div className={`relative z-10 max-w-md w-full backdrop-blur-2xl border rounded-[3.5rem] p-10 shadow-2xl transition-all duration-500 animate-in fade-in zoom-in
        ${isDarkMode 
        ...
        <div className="text-center mb-10">
           ...
           <p className={`mt-2 font-black tracking-[0.3em] uppercase text-[10px] opacity-50 
             ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Clinical Rehab AI</p>
           <h2 className={`text-2xl font-black mt-6 tracking-tight ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>
             {t('welcome_back')}
           </h2>
        </div>
        
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" dir={i18n.language === 'ar' ? 'rtl' : 'ltr'}>
          <div className="relative group">
            <User className={`absolute ${i18n.language === 'ar' ? 'right-5' : 'left-5'} top-5 transition-colors 
            ...
            <input
              ...
              className={`w-full ${i18n.language === 'ar' ? 'pr-14 pl-6' : 'pl-14 pr-6'} py-5 rounded-2xl outline-none transition-all font-bold placeholder:opacity-50
                ${isDarkMode 
          ...
          <div className="relative group">
            <Lock className={`absolute ${i18n.language === 'ar' ? 'right-5' : 'left-5'} top-5 transition-colors 
              ${isDarkMode ? 'text-slate-500 group-focus-within:text-blue-400' : 'text-slate-400 group-focus-within:text-blue-600'}`} size={20} />
            <input
          ...
          <button 
            disabled={loading}
            type="submit"
            className={`w-full py-5 rounded-2xl font-black text-xl transition-all active:scale-95 flex items-center justify-center gap-3 shadow-xl
              ${isDarkMode 
      ...
      <div className={`absolute bottom-8 w-full justify-between px-12 opacity-20 hidden lg:flex ${i18n.language === 'ar' ? 'flex-row' : 'flex-row-reverse'}`}>
         <p className={`${isDarkMode ? 'text-white' : 'text-slate-900'} text-[10px] font-black uppercase tracking-[0.5em]`}>Staqem Encryption Active</p>
         <p className={`${isDarkMode ? 'text-white' : 'text-slate-900'} text-[10px] font-black uppercase tracking-[0.5em]`}>© 2026 Professional Care</p>
      </div>
    </div>
  );
};
